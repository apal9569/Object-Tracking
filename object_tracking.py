...
def main(_argv):
    cap, ZED_FLAG = initialize_video_capture(FLAGS.video)
    ...
    if ZED_FLAG:
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            zed.retrieve_image(cap, sl.VIEW.LEFT)
            shape = np.shape(cap.get_data())
            frame_width, frame_height = shape[1], shape[0]
            fps = 30
    else:
